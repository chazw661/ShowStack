...
function openGroupPicker(assignmentId, sessionId, dotEl) {
    ...
    var picker = document.getElementById('group-picker-' + assignmentId) || document.getElementById('group-picker-a2-' + assignmentId);
    if (!picker) return;
...
function assignGroup(assignmentId, groupId, groupName, groupColor, dotEl) {
    ...
    .then(function(data) {
        if (data.success) {
            ...
            // Also update all a1-slot-rows for this assignment
            document.querySelectorAll('tr[data-assignment-id="' + assignmentId + '"]').forEach(function(slotRow) {
                slotRow.classList.remove('group-blue', 'group-amber', 'group-red', 'group-purple', 'group-teal');
                if (groupColor) slotRow.classList.add('group-' + groupColor);
                var dot = slotRow.querySelector('.group-dot');
                if (dot) { dot.className = 'group-dot ' + (groupColor ? 'color-' + groupColor : 'empty'); dot.title = groupName || 'Assign group'; }
            });
        }
    });
}
...
function openSlotGroupPicker(slotId, sessionId, dotEl) {
    document.querySelectorAll('.group-picker.open').forEach(p => p.classList.remove('open'));
    ...
    loadGroups(sessionId, function(groups) {
        ...
        noneItem.onmousedown = function(e) {
            e.preventDefault();
            assignSlotGroup(slotId, null, null, null, dotEl);
            picker.classList.remove('open');
        };
        picker.appendChild(noneItem);
        if (groups.length > 0) {
        ...
        groups.forEach(function(g) {
            ...
            item.onmousedown = function(e) {
                e.preventDefault();
                assignSlotGroup(slotId, g.id, g.name, g.color, dotEl);
                picker.classList.remove('open');
            };
            picker.appendChild(item);
        });
...
function assignSlotGroup(slotId, groupId, groupName, groupColor, dotEl, assignmentIdOverride) {
    ...
    .then(function(data) {
        if (data.success) {
            dotEl.className = 'group-dot ' + (groupColor ? 'color-' + groupColor : 'empty');
            dotEl.title = groupName || 'Assign group';
            var row = document.getElementById('a1-slot-row-' + slotId);
            if (row) {
                row.classList.remove('group-blue','group-amber','group-red','group-purple','group-teal');
                if (groupColor) row.classList.add('group-' + groupColor);
            }
