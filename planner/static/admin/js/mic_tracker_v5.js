...
// ── Multi-group picker helpers ─────────────────────────────────
var GROUP_TINT_CLASSES = ['group-blue', 'group-amber', 'group-red', 'group-purple', 'group-teal'];

function readSelectedGroupIds(stackEl) {
    var raw = stackEl && stackEl.dataset.groupIds ? stackEl.dataset.groupIds : '';
    if (!raw) return [];
    return raw.split(',').filter(Boolean).map(function(x) { return parseInt(x, 10); });
}

function renderDotStack(stackEl, selectedGroups, dotClass) {
    if (!stackEl) return;
    // Inline the layout styles so external CSS load order or caching can't
    // break the row-flex layout — every dot is forced 8×8 with no border.
    stackEl.style.display = 'inline-flex';
    stackEl.style.flexDirection = 'row';
    stackEl.style.flexWrap = 'nowrap';
    stackEl.style.alignItems = 'center';
    stackEl.style.gap = '2px';
    stackEl.style.verticalAlign = 'middle';
    stackEl.innerHTML = '';
    var dotStyle = 'width:8px;height:8px;border:0;flex:0 0 auto;box-sizing:content-box;display:inline-block;';
    if (!selectedGroups.length) {
        var empty = document.createElement('span');
        empty.className = dotClass + ' empty';
        stackEl.appendChild(empty);
    } else {
        selectedGroups.forEach(function(g) {
            var d = document.createElement('span');
            d.className = dotClass + ' color-' + g.color;
            d.setAttribute('style', dotStyle + (dotClass === 'a2-group-dot' ? 'margin-left:0;' : ''));
            stackEl.appendChild(d);
        });
    }
    stackEl.dataset.groupIds = selectedGroups.map(function(g) { return g.id; }).join(',');
    stackEl.title = selectedGroups.length
        ? selectedGroups.map(function(g) { return g.name; }).join(', ')
        : 'Assign group';
}

function applyRowTint(els, firstColor) {
    els.forEach(function(el) {
        if (!el) return;
        GROUP_TINT_CLASSES.forEach(function(c) { el.classList.remove(c); });
        if (firstColor) el.classList.add('group-' + firstColor);
    });
}

function buildMultiPicker(picker, groups, getSelectedIds, onToggle, onClearAll) {
    picker.innerHTML = '';
    var title = document.createElement('div');
    title.className = 'group-picker-title';
    title.textContent = 'Assign Groups';
    picker.appendChild(title);

    var clear = document.createElement('div');
    clear.className = 'group-picker-none';
    clear.textContent = '— Clear All —';
    clear.onmousedown = function(e) { e.preventDefault(); onClearAll(); };
    picker.appendChild(clear);

    if (groups.length) {
        var divider = document.createElement('div');
        divider.className = 'group-picker-divider';
    ...
    groups.forEach(function(g) {
        ...
        cb.type = 'checkbox';
        cb.checked = selected.indexOf(g.id) >= 0;
        cb.style.cssText = 'margin:0;width:13px;height:13px;flex-shrink:0;cursor:pointer;';
        cb.onchange = function() { onToggle(g, cb.checked); };
        item.appendChild(cb);
        var swatch = document.createElement('span');
        swatch.style.cssText = 'width:12px;height:12px;border-radius:50%;background:' + GROUP_COLORS[g.color] + ';display:inline-block;flex-shrink:0;';
...
function positionPicker(picker, anchorRect) {
    ...
    if (window.innerHeight - anchorRect.bottom < 240) {
        picker.style.top = (anchorRect.top - 4) + 'px';
...
function attachOutsideCloser(picker, anchorEl) {
    function handler(e) {
...
// ── Single-presenter A1 row picker (writes assignment.groups) ──
function openGroupPicker(assignmentId, sessionId, stackEl) {
    ...
    if (!picker) return;
    var anchorRect = stackEl.getBoundingClientRect();
    loadGroups(sessionId, function(groups) {
        var groupsById = {};
        groups.forEach(function(g) { groupsById[g.id] = g; });
        var selectedIds = readSelectedGroupIds(stackEl);

        function commit() {
            fetch('/audiopatch/api/mic-groups/assign/', {
                method: 'POST',
                headers: {'Content-Type': 'application/json', 'X-CSRFToken': getCsrfToken()},
                body: JSON.stringify({assignment_id: assignmentId, group_ids: selectedIds})
            })
            .then(r => r.json())
            .then(function(data) {
                if (!data.success) return;
                selectedIds = (data.group_ids || []).slice();
        ...
        buildMultiPicker(picker, groups,
            function() { return selectedIds; },
            function(g, checked) {
                if (checked) { if (selectedIds.indexOf(g.id) < 0) selectedIds.push(g.id); }
                else { selectedIds = selectedIds.filter(function(id) { return id !== g.id; }); }
                commit();
            },
            function() {
                selectedIds = [];
        ...
        positionPicker(picker, anchorRect);
        picker.classList.add('open');
        attachOutsideCloser(picker, stackEl);
    });
}
...
async function addPresenterSlot(assignmentId) {
    ...
    try {
        ...
        const data = await resp.json();
        if (data.success) {
            // Inject new chip into the slot list without reloading
            const slotList = document.querySelector(`#a2-card-${assignmentId} .a2-slot-list`);
            const queueEl = document.querySelector(`#a2-card-${assignmentId} .a2-slot-queue`);
            if (slotList) {
                const chip = document.createElement('div');
                chip.className = 'a2-slot-chip';
                chip.dataset.slotId = data.slot_id;
                chip.innerHTML = `Unassigned <span class="a2-slot-remove" onclick="event.stopPropagation(); removePresenterSlot(${assignmentId}, ${data.slot_id})">✕</span>`;
                chip.onclick = () => activateSlot(assignmentId, data.slot_id, chip);
                slotList.appendChild(chip);
            } else if (queueEl) {
                // slot list doesn't exist yet — reload to build full queue UI
                location.reload();
                return;
            } else {
                location.reload();
            ...
            // Show queue section if hidden
            if (queueEl) queueEl.style.display = '';
        } else {
            alert('Failed to add presenter: ' + (data.error || 'Unknown error'));
        }
    } catch(e) {
        console.error('addPresenterSlot error:', e);
        alert('Error adding presenter slot');
    }
}

// ── Multi-presenter A1 row picker (writes PresenterSlot.groups) ──
function openSlotGroupPicker(slotId, sessionId, stackEl) {
    document.querySelectorAll('.group-picker.open').forEach(p => p.classList.remove('open'));
    var picker = document.getElementById('slot-group-picker-' + slotId);
    if (!picker) return;
    var anchorRect = stackEl.getBoundingClientRect();
    loadGroups(sessionId, function(groups) {
        var groupsById = {};
        ...
        function commit() {
            fetch('/audiopatch/api/mic/slot/assign-group/', {
                method: 'POST',
                headers: {'Content-Type': 'application/json', 'X-CSRFToken': getCsrfToken()},
                body: JSON.stringify({slot_id: slotId, group_ids: selectedIds})
            })
            .then(r => r.json())
            .then(function(data) {
                if (!data.success) return;
                ...
                renderDotStack(stackEl, selectedGroups, 'group-dot');
                var firstColor = selectedGroups[0] ? selectedGroups[0].color : null;
                applyRowTint([document.getElementById('a1-slot-row-' + slotId)], firstColor);
            });
        }
...
// ── A2 card picker (writes active PresenterSlot.a2_groups) ──
function openA2GroupPicker(assignmentId, sessionId, stackEl) {
    document.querySelectorAll('.group-picker.open').forEach(p => p.classList.remove('open'));
    var picker = document.getElementById('a2-group-picker-' + assignmentId);
    if (!picker) return;
    var slotId = stackEl.dataset.activeSlotId;
    ...
    loadGroups(sessionId, function(groups) {
        ...
        function commit() {
            ...
            .then(function(data) {
                ...
                selectedIds = (data.group_ids || []).slice();
                var selectedGroups = selectedIds.map(function(id) { return groupsById[id]; }).filter(Boolean);
                renderDotStack(stackEl, selectedGroups, 'a2-group-dot');
                var firstColor = selectedGroups[0] ? selectedGroups[0].color : null;
